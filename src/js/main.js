...
function preloadImages() {
  ...
  const loadImage = async (src) => {
        const blob = await fetch(src, {mode: "no-cors"}).then(res => res.blob());
  ...
  return Promise.all(characterDataToSort.map(async (char, idx) => {
    if (document.getElementById("cb-noscreenshots").checked == false) {
        characterDataToSort[idx].img = await loadImage(imageRoot + char.img);
    }
    else {
        characterDataToSort[idx].img = "src/assets/defaultR.png";
    }
  }));
}
